...
describe("GET /api/briefing", () => {
    ...
    it("lets a viewer read the snapshot", async () => {
        ...
        assert.equal(response.body.data.open_alerts, undefined);
        assert.equal(response.body.data.offline_vehicles, undefined);
        assert.equal(response.body.data.drivers, undefined);
    });
