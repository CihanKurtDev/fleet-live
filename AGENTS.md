...
- README mentions alerts as a domain (table, `active_alerts`, filter) and as REST (`GET`/`PATCH /api/alerts` with optional `type`). SPEEDING, LOW_FUEL and OFFLINE rows are live ticker events (8 s over the current sim road-class limit; fuel under 15% while `DRIVING`; no report for 15 s after a company pause, or status `OFFLINE`). That is not OSM and not a general rule engine. Home `/` is the shift briefing (`GET /api/briefing`). Next engineering item is Phase 9.2.2 (separate import sheets).
